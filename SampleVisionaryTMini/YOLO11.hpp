...
void YOLO11Detector::initialize(const std::string& modelPath, const std::string& labelsPath, bool useGPU, int gpuIndex) {
    ...
    sessionOptions.SetIntraOpNumThreads(1);//std::min(6, static_cast<int>(std::thread::hardware_concurrency())));
    sessionOptions.SetInterOpNumThreads(1);
    sessionOptions.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);//ORT_ENABLE_ALL);
